...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_product_variants_stock ON product_variants(stock_quantity);

-- Exchange rates indexes (for currency conversion performance)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_exchange_rates_from_currency ON exchange_rates(from_currency_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_exchange_rates_to_currency ON exchange_rates(to_currency_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_exchange_rates_effective_date ON exchange_rates(effective_date DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_exchange_rates_lookup ON exchange_rates(from_currency_id, to_currency_id, effective_date DESC);

-- Currencies indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_currencies_code ON currencies(code) WHERE is_active = true;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_currencies_is_active ON currencies(is_active);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_products_active_category ON products(is_active, category_id, created_at DESC);
