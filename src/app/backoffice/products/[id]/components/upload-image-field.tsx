...
import { DetailProductResponse } from "../data/data-fetching";
...
export function UploadImageField() {
  ...
  const data = queryClient.getQueryData<DetailProductResponse>([
    "product-detail",
    id,
  ]);
    ...
    productImages.forEach((image) => {
      dispatch({
        ...
        payload: [
          {
            id: image,
            preview: `${process.env.NEXT_PUBLIC_BUCKET_URL}/${image}`,
            file: null,
          },
        ],
      });
      dispatch({ type: "SET_UPLOADED", payload: image });
    });
  }, [data?.data.images]);
